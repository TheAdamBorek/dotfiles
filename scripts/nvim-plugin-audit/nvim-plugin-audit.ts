...
type Action =
  ...
  | { kind: "cooled"; old: string; new: string; cooled: string; newAgeHours: number; cooledAgeHours: number }
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === "--skip-sync") args.skipSync = true;
  ...
  --dry-run            report only; don't write the lock file
...
async function sh(cwd: string, cmd: string[]): Promise<{ stdout: string; ok: boolean; stderr: string }> {
...
async function commitTimestamp(repo: string, sha: string): Promise<number | null> {
  const r = await sh(repo, ["git", "log", "-1", "--format=%ct", sha]);
  if (!r.ok || !r.stdout) return null;
  return Number(r.stdout);
}

async function findCooledSha(repo: string, branch: string, cutoffEpoch: number): Promise<string | null> {
  const r = await sh(repo, ["git", "rev-list", "-1", `--before=${cutoffEpoch}`, `origin/${branch}`]);
  if (r.ok && r.stdout) return r.stdout;
  const r2 = await sh(repo, ["git", "rev-list", "-1", `--before=${cutoffEpoch}`, branch]);
...
async function runLazySync() {
  ...
  const p = Bun.spawn(["nvim", "--headless", "+Lazy! sync", "+qa"], { stdout: "inherit", stderr: "inherit" });
  const code = await p.exited;
  if (code !== 0) {
...
async function main() {
  ...
  for (const name of [...names].sort()) {
    ...
    if (!o && n) {
      const ts = existsSync(repo) ? await commitTimestamp(repo, n.commit) : null;
      const age = ts ? (now - ts) / 3600 : -1;
      reports.push({ plugin: name, action: { kind: "new", sha: n.commit, ageHours: age } });
      continue;
    }
    ...
    if (!existsSync(repo)) {
      reports.push({ plugin: name, action: { kind: "error", message: `repo not found at ${repo}` } });
      continue;
    }

    const newTs = await commitTimestamp(repo, n.commit);
    if (newTs === null) {
      reports.push({ plugin: name, action: { kind: "error", message: `cannot read commit time for ${n.commit}` } });
      continue;
    }
    ...
    if (newTs <= cutoff) {
      reports.push({ plugin: name, action: { kind: "kept", old: o.commit, new: n.commit, ageHours: newAge } });
      continue;
    }

    const cooled = await findCooledSha(repo, n.branch, cutoff);
    if (!cooled) {
      reports.push({ plugin: name, action: { kind: "error", message: `no commit older than cooldown on branch ${n.branch}` } });
      continue;
    }
  ...
  if (args.dryRun) {
    console.log("\n(dry-run) lock not modified.");
    return;
  }

  await writeFile(args.lockPath, JSON.stringify(cooledLock, null, 2) + "\n");
  console.log(`\n✓ wrote cooled lock → ${args.lockPath}`);
  console.log("  next time you open nvim, lazy will check out the cooled SHAs.");
}
...
function printReport(reports: Report[], cooldown: number) {
  ...
  for (const { plugin, action } of changed) {
    ...
    switch (action.kind) {
      case "new":
        console.log(`+ ${plugin}  NEW  ${short(action.sha)}  age=${fmtAge(action.ageHours)}`);
        break;
      case "removed":
        console.log(`- ${plugin}  REMOVED`);
        break;
      case "kept":
        console.log(`✓ ${plugin}  ${short(action.old)} → ${short(action.new)}  age=${fmtAge(action.ageHours)}`);
        break;
      case "cooled":
        console.log(
